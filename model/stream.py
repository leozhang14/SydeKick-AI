...
# -------- LIVE CAMERA STREAMING FUNCTION --------
def process_camera(camera_index=0):
    ...
    print(f"Starting live camera feed (device {camera_index})...")
    ...
    if not cap.isOpened():
        raise ValueError(f"Failed to open camera device {camera_index}")
    
    # Use default FPS for camera (typically 30)
    fps = FPS
    ...
    try:
        while True:
            ...
            # Display frame with prediction (optional)
            display_frame = frame.copy()
            cv2.putText(display_frame, f"{pred_class}: {confidence:.2f}", 
                       (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            cv2.putText(display_frame, f"Frame: {frame_count}", 
                       (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            cv2.imshow("UFC Action Detection", display_frame)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print(f"Warning: Invalid camera index '{sys.argv[1]}', using default camera 0")
            camera_index = 0
